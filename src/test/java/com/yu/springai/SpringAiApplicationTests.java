...
import com.yu.springai.utils.VectorDistanceUtils;
...
class SpringAiApplicationTests {
    ...
    public void testPDFLoader(){
        ...
        // 3.写入向量库
        documents.forEach(document -> { // 添加元数据
            document.getMetadata().put("file_name","中二知识笔记.pdf");
        });
    ...
    public void testVectorStore() {
        ...
        vectorStore.add(List.of(
                new org.springframework.ai.document.Document("我想吃炸鸡")
        ));

        // 查询
        List<Document> results = vectorStore.similaritySearch(SearchRequest.builder().query("pinecone 配置").topK(3).build());
    ...
    @Test
    void contextLoads() {
        float[] embed = embeddingModel.embed("早饭吃牛奶面包");
